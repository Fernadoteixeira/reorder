...
Essa habilidade ajuda o agente de IA a escrever, executar e verificar testes para o projeto de `reordenação` de maneira eficaz.
...
4. **Testes independentes**: Os testes não devem depender de dados pré-projetados no banco de dados ou dos resultados de outros testes. Prepare todos os equipamentos necessários no bloco `beforeAll` / `beforeEach` e limpe depois.
