...
A área `Settings` fornece configuração de tempo de execução global para comportamento de comércio recorrente gerenciado pelo plugin.
...
- fornecer entradas de configuração estáveis para `Subscriptions`, `Renewals`, `Dunning` e `Cancellation & Retention`
...
Isso significa que a área `Settings` é um domínio de configuração, não um domínio operacional.
...
A área `Settings` segue estas regras de fonte da verdade:
...
Mas `Activity Log` não é a fonte da verdade para o próprio registro de configurações.
...
Estes são valores que devem ser lidos de `SubscriptionSettings` quando uma nova operação for iniciada.
...
- uma recomendação ou caminho padrão já persistido em `CancellationCase`
...
Para MVP, `SubscriptionSettings` use um modelo leve de changelog.
...
- `metadata.last_update` como um instantâneo conveniente das últimas alterações
...
Isso significa:
- ainda não adicionamos uma entidade separada somente para acréscimos `settings_change`
...
- `metadata.audit_log` continua sendo a fonte da verdade para o histórico de alterações leves
...
- eventos de negócios de assinatura em `Activity Log`
...
`change_summary` foi projetado para permanecer compacto e legível pelo operador.
...
então a próxima etapa correta é um registro `settings_change` dedicado somente para acréscimos.
...
Para MVP a área `Settings` deverá seguir esta decisão:
...
- comportamento operacional seguro para `Renewals`, `Dunning` e `Cancellation & Retention`

## Fonte de verdade em tempo de execução e Bootstrap

A fonte da verdade em tempo de execução implementada é:
- registro singleton `subscription_settings` persistente no módulo `settings`
...
- emitir logs `settings.update` estruturados
...
- as gravações posteriores devem corresponder ao `version` persistente atual
...
`Renewals` lê as configurações efetivas quando um novo `RenewalCycle` futuro é criado.
...
Para MVP, `default_renewal_behavior` deve usar esta enumeração:
...
- ignorando regras de aprovação já armazenadas em `RenewalCycle`
...
Significado:
- quando uma operação de renovação é iniciada e há um contexto de mudança revisável, como `pending_update_data`, o sistema deve adotar como padrão um caminho de revisão/aprovação

Isso não significa:
- reescrevendo o estado de aprovação em um `RenewalCycle` já criado
- ciclos alterados retroativamente já persistidos no estado de tempo de execução

## `SubscriptionCancellationBehavior`

Para MVP, `default_cancellation_behavior` deve usar esta enumeração:
...
Para MVP, a área `Settings` deve usar uma única hierarquia de fonte de verdade em tempo de execução:
...
A principal fonte de verdade para `SubscriptionSettings` deveria ser:
...
## `GET` Comportamento quando o registro está faltando
...
- Admin `GET` ainda funciona por meio de configurações efetivas de fallback
...
- é autoritário
- carrega `version`, `updated_by` e `updated_at` persistidos reais
...
- `subscription_settings` no banco de dados é a fonte primária de tempo de execução
