...
Recommended wording for the Configurações UI:
...
- the Configurações page save UX
...
Configurações writes are implemented through the dedicated workflow:
...
The Configurações UI should live under Medusa Admin `Settings`, not under `Subscriptions`.
...
## Fonte da Verdade and Bootstrap
...
## Effective Configurações Semantics

The system should distinguish between:
- persisted settings
- effective fallback settings

### Persisted Configurações
...
### Effective Fallback Configurações
