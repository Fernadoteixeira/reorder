...
- apoiar a integração do nível de fumaça em `Dunning`, `Renewals`, `Subscriptions` e `Cancellation & Retention`
...
- `start-dunning` caminho de sucesso
- atualização idempotente de um caso existente para o mesmo ciclo de renovação
- caso ativo duplicado bloqueado
- `run-dunning-retry` caminho de recuperação
- `run-dunning-retry` falha temporária e caminho de reagendamento
...
- `insufficient_funds`, `generic_decline` e `do_not_honor` podem ser repetidas e devem reagendar o caso
- `requires_more`, forma de pagamento ausente e detalhes de pagamento expirados são terminais e devem encerrar o caso como `unrecovered`
...
- a recuperação bem-sucedida do pagamento encerra o caso e restaura a assinatura para `active`
...
- `past_due` assinaturas ainda podem entrar em fluxos de cancelamento e retenção
- `DunningCase` ativo permanece visível e não é controlado por fluxos de trabalho de cancelamento
...
- estender a verificação de fumaça quando as alterações afetarem o limite do tempo de execução com `Renewals`, `Subscriptions` ou `Cancellation & Retention`
...
A área `Dunning` é atualmente testada por meio de camadas de integração HTTP suportadas pela Medusa, em vez de automação do navegador.
...
- o limite de integração de tempo de execução com `Renewals`, `Subscriptions` e `Cancellation & Retention`
