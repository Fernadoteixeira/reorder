...
Este documento descreve a estratégia de teste atual para a área `Renewals` no plugin `Reorder`.
...
A área `Renewals` é testada em duas camadas principais:
...
Objetivo:
- verificar o serviço do módulo `renewal` isoladamente dos fluxos administrativos completos
...
- apoiar a integração do nível de fumaça em `Renewals`, `Subscriptions`, `Plans & Offers` e `Cancellation & Retention`
...
- a renovação não ignora a política `Plans & Offers` ativa
- a falha no pagamento da renovação qualificada começa em `Dunning`
- a execução da renovação futura respeita os efeitos do ciclo de vida provenientes de `Cancellation & Retention`
...
- estender a verificação de fumaça quando as alterações afetarem a integração com `Subscriptions`, `Plans & Offers` ou `Cancellation & Retention`

Para a nova funcionalidade `Renewals`:
...
4. atualize a verificação de fumaça se a semântica de renovação mudar no limite com `Subscriptions`, `Plans & Offers` ou `Cancellation & Retention`
...
A área `Renewals` é atualmente testada por meio de camadas de integração suportadas pela Medusa, em vez de automação do navegador.
...
- o limite de integração com `Subscriptions` e `Plans & Offers`
