...
- [Assinaturas Testing](./subscriptions.md)
- [Renovações Testing](./renewals.md)
- [Dunning (Cobrança) Testing](./dunning.md)
- [Cancellations Testing](./cancellations.md)
- [Renovações Admin UI](../admin/renewals.md)
- [Dunning (Cobrança) Admin UI](../admin/dunning.md)
- [Cancellations Admin UI](../admin/cancellations.md)
- [Registro de Atividades Admin UI](../admin/activity-log.md)
- [Análise de Dados e Métricas Admin UI](../admin/analytics.md)
- [Assinatura Configurações Admin UI](../admin/subscription-settings.md)
- [Renovações Architecture](../architecture/renewals.md)
- [Dunning (Cobrança) Architecture](../architecture/dunning.md)
- [Cancelamento Architecture](../architecture/cancellation.md)
- [Registro de Atividades Architecture](../architecture/activity-log.md)
- [Análise de Dados e Métricas Architecture](../architecture/analytics.md)
- [Configurações Architecture](../architecture/settings.md)
...
- if no persisted singleton exists, the seed creates one for Configurações QA
- if the existing singleton was previously created by this seed, the seed updates it deterministically
- if a non-seeded singleton already exists, the script keeps it untouched and skips the Configurações-specific QA seed instead of failing or overwriting store configuration
...
- one global `SubscriptionSettings` singleton for Configurações QA
- two `Plan Offers`
...
- deterministic `subscription_metrics_daily` rows for Análise de Dados e Métricas QA
...
### 0. Configurações global defaults

Scope:
- global `SubscriptionSettings` singleton

Purpose:
- validate the Configurações page in `Settings -> Subscription Settings`
...
- because Configurações are global, the seed does not overwrite an existing non-seeded singleton
- in that case the script logs a warning and skips the Configurações-only QA scenario while still seeding the rest of the Assinaturas workspace

## Análise de Dados e Métricas QA Window

For manual Análise de Dados e Métricas QA, the seeded snapshot window is:
...
The Análise de Dados e Métricas data is intentionally snapshot-first.
...
### 1. Renovação success without approval
...
### 6. Falhou history / retry inspection
...
### 7. Dunning (Cobrança) queue: retry scheduled
...
### 8. Dunning (Cobrança) detail: awaiting manual resolution
...
### 9. Dunning (Cobrança) history: recovered
...
## Análise de Dados e Métricas Scenarios

The current seed also creates these Análise de Dados e Métricas-oriented QA scenarios:

### 10. Análise de Dados e Métricas overview baseline
...
### 11. Análise de Dados e Métricas frequency comparison
...
Implementation note:
- this subscription is created only to make Análise de Dados e Métricas cadence filtering deterministic
- it is not meant to drive renewal-flow QA

### 12. Análise de Dados e Métricas status segmentation
...
Purpose:
- validate `status` filtering in Análise de Dados e Métricas
...
### 13. Análise de Dados e Métricas churn windows
...
### 14. Análise de Dados e Métricas MRR anomaly day
...
- this is meant for Análise de Dados e Métricas QA only

## Recommended Análise de Dados e Métricas QA Checks
...
- if your store already had its own non-seeded global settings:
  ...
  - expect no Configurações-specific QA override to be applied
- open `Subscriptions -> Analytics` with the default Análise de Dados e Métricas QA window
...
### 10. Dunning (Cobrança) history: unrecovered

### 11. Registro de Atividades: admin subscription event
...
### 12. Registro de Atividades: scheduler renewal event
...
### 13. Registro de Atividades: system dunning event
...
### 11. Dunning (Cobrança) detail: manual retry-schedule override
...
### 12. Cancelamento: open billing case with active dunning
...
### 13. Cancelamento: retained after discount offer
...
### 14. Cancelamento: paused after pause offer
...
### 15. Cancelamento: immediate final cancel
...
### 16. Cancelamento: end-of-cycle final cancel
...
### 17. Cancelamento: open price-driven case
...
### 18. Cancelamento: open case on already paused subscription
