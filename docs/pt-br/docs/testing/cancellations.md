...
Este documento descreve a estratégia de teste atual para a área `Cancellation & Retention` no plugin `Reorder`.
...
A configuração de teste para `Cancellation & Retention` foi projetada para proteger o plugin nas camadas oficialmente suportadas pelas ferramentas de teste da Medusa.
...
A área `Cancellation & Retention` está atualmente testada na camada de integração de aplicativos Medusa.

Isso dá cobertura para:
- fluxos de trabalho
- rotas personalizadas da API Admin
- Fluxos de back-end de administração usados pela IU
- integração de nível de fumaça com `Subscriptions`, `Renewals` e `Dunning`
...
- apoiar a integração do nível de fumaça em `Cancellation & Retention`, `Subscriptions`, `Renewals` e `Dunning`
...
- `start-cancellation-case` caminho de sucesso
- reutilização ou atualização idempotente de um caso ativo existente
- guarda de caso ativa duplicada
- `apply-retention-offer` caminhos de sucesso
- `apply-retention-offer` falhas de política
- `finalize-cancellation` caminho de sucesso
- `finalize-cancellation` comportamento de motivo obrigatório
- Comportamento `update-cancellation-reason`
...
- pausar a retenção atualiza o ciclo de vida da assinatura para `paused`
...
- cancelamento final define `cancel_effective_at` e limpa `next_renewal_at`
- `past_due` assinaturas ainda podem entrar no fluxo de cancelamento
- `DunningCase` ativo coexiste com fluxo de cancelamento ativo sem sobreposição de propriedade
...
- estender a verificação de fumaça quando as alterações afetarem o limite do tempo de execução com `Subscriptions`, `Renewals` ou `Dunning`

Para a nova funcionalidade `Cancellation & Retention`:
...
Ao alterar a área `Cancellation & Retention`:
...
3. atualize a verificação de fumaça se a semântica de cancelamento mudar no limite com `Subscriptions`, `Renewals` ou `Dunning`
...
A área `Cancellation & Retention` é atualmente testada por meio de camadas de integração HTTP suportadas pela Medusa, em vez de automação do navegador.
...
- o limite de integração de tempo de execução com `Subscriptions`, `Renewals` e `Dunning`
