...
Este documento descreve a estratégia de teste atual para a área `Plans & Offers` no plugin `Reorder`.
...
A configuração de teste para `Plans & Offers` foi projetada para proteger o plugin nas camadas oficialmente suportadas pelas ferramentas de teste da Medusa.
...
A área `Plans & Offers` é testada em duas camadas principais:
...
- integração entre áreas com `Subscriptions`
...
- suporte à integração do nível de fumaça com `Subscriptions`
...
A estratégia de teste atual também inclui integração de nível de fumaça com `Subscriptions`.
...
- estender a verificação de fumaça `Subscriptions` quando alterações na semântica da oferta afetarem alterações no plano de assinatura

Para a nova funcionalidade `Plans & Offers`:
...
Ao alterar a área `Plans & Offers`:
...
4. atualize a verificação de fumaça `Subscriptions` se a configuração efetiva ou o comportamento de mudança de plano mudarem
...
A área `Plans & Offers` é atualmente testada por meio de camadas de integração suportadas pela Medusa, em vez de automação do navegador.
