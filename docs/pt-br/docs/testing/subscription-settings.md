...
Este documento descreve a estratégia de teste atual para a área `Subscription Settings` no plugin `Reorder`.
...
A área `Subscription Settings` está atualmente protegida em cinco camadas:
...
- falhas de validação
- `409 conflict` para `expected_version` obsoleto
...
Esta camada cobre:
- Configurações de leitura de `Dunning` para nova criação de `DunningCase`
- `Cancellation` capturando o comportamento padrão do novo `CancellationCase`
- `Renewals` usando configurações no momento da criação
...
- efeito de tempo de execução após a alteração das configurações por meio de um fluxo `Dunning` representativo
...
Coberto:
- Integração `Dunning`
- Integração `Cancellation`
- `Renewals` semântica de snapshot no momento da criação

### Cobertura do fluxo administrativo

Coberto:
- fluxo principal do operador para leitura e salvamento de configurações
...
`Subscription Settings` estão atualmente protegidos nas camadas que são importantes para este plugin:
- correção do módulo
