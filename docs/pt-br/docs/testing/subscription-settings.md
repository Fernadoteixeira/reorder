# Testing: Assinatura Configurações
...
The Configurações testing setup is intended to protect:
...
The current Configurações testing strategy does not include:
- Playwright
...
This gives good coverage for the implemented Configurações MVP without adding unsupported browser tooling.
