# Roadmap: Assinatura Configurações
...
- runtime integration for:
  ...
  - `Cancellation`
  - `Renewals`
- Admin Configurações page for read and update flows
...
Current state:
- the Configurações page shows current record state
...
- explicit confirmation UX on the Configurações page
...
### 4. Add Per-Store Configurações Scope
...
This is consistent with the current plugin maturity level and with the architecture already chosen for the Configurações feature.
