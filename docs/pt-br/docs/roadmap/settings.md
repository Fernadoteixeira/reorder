...
Este documento captura os limites atuais do MVP e a evolução planejada da área `Subscription Settings` no plugin `Reorder`.
...
- adicionar um modelo de permissão completo apenas para `Subscription Settings` introduziria uma nova preocupação transversal no final do MVP
...
- `version` suporta bloqueio e ordenação otimistas
...
A direção atual do roteiro para `Subscription Settings` é:
