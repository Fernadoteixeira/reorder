...
- definir quando `pending_update_data` é apenas um estado de visualização e quando se torna uma entrada executável
- definir quando a aprovação é necessária
- definir como `Planos e Ofertas` afetam a execução da renovação
...
A área de “Renovações” tem três preocupações distintas:
...
O estado de execução do ciclo de renovação reside no módulo `renovação`.
...
A validação da política comercial atual é derivada de `Planos e Ofertas`.
...
A fonte da verdade para a assinatura do cliente continua sendo a `Assinatura`.
...
A fonte da verdade para o histórico de novas tentativas e execuções continua sendo `RenewalAttempt`.

As tentativas não substituem o estado agregado do ciclo.

### Fonte da verdade da política de oferta

A fonte da verdade para frequências permitidas e regras de oferta continua sendo `PlanOffer`.
...
- `pending_update_data` aprovado e aplicável
- configuração efetiva atual de `Planos e Ofertas` apenas para validação, não como a principal fonte de carga útil do pedido

### Campos de origem primários

O fluxo de renovação deve tratar estes campos `Assinatura` como entrada principal:
- `subscrição.id`
- `estado`
- `id_produto`
- `variant_id`
...
`customer_snapshot` é usado como contexto de renovação estável do cliente.
...
`product_snapshot` é usado como o instantâneo de comércio de assinatura estável.
...
- que a validação da política pertence a `Planos e Ofertas`
...
`pending_update_data` pertence a `Subscrição`.
...
- essas alterações devem ser materializadas de volta ao estado ativo `Assinatura`
...
A aprovação deve ser necessária apenas quando um ciclo aplicar `pending_update_data` e a política de negócios diz que a mudança deve ser revisada antes da execução.

Nesta fase, o contrato da fonte da verdade deve apoiar:
- `approval_status = null` quando nenhuma aprovação é necessária
- `approval_status = pendente` quando a aprovação é necessária e está indecisa
- `approval_status = aprovado` quando o ciclo pode aplicar a mudança
- `approval_status = rejeitado` quando o ciclo não deve aplicar a alteração
...
## 8. Como `Planos e Ofertas` afetam a renovação

Os `Planos e Ofertas` devem influenciar as renovações como validação da política atual, e não como principal reserva do estado de renovação.
...
`Planos e Ofertas` não devem substituir:
...
O ciclo de renovação não deve manter uma cópia completa do `ProductSubscriptionConfig` como sua principal fonte de verdade.
...
- quais mudanças futuras estão programadas como estado de visualização
...
- `last_renewal_at` na assinatura deve avançar
...
- o ciclo não deve aplicar `pending_update_data`
...
- `Assinatura` = verdade da assinatura atual
...
Cada preocupação tem seu próprio proprietário e deve permanecer separada.
...
- tratar `pending_update_data` como estado de assinatura já ativo
