...
Objetivo:
- definir a divisão da fonte da verdade entre `Assinaturas`, `Renovações` e `Dunning`
...
O `DunningCase` deve ser ancorado em um evento concreto de dívida falida originado de um `RenewalCycle`.
...
Tentativas posteriores com falha devem atualizar o mesmo `DunningCase`, e não criar um novo caso.
...
- cada tentativa de recuperação posterior pertence ao mesmo caso até que feche como `recuperado` ou `não recuperado`
...
### 8.1 Estado de propriedade de `Assinatura`
...
### 8.2 Estado de propriedade da `RenewalCycle`

`RenewalCycle` possui:
- data de vencimento e cronograma de execução para um evento de faturamento
- status de execução como `agendado`, `processamento`, `sucesso`, `falha`
...
### 8.3 Estado de propriedade de `DunningCase`
...
- carimbos de data e hora de recuperação, como `recovered_at` ou `closed_at`
...
- a política comercial de origem de `Planos e Ofertas`
...
- o `RenewalCycle` de origem não deixa de ser a fonte da tentativa original de execução falhada
- o `DunningCase` torna-se a fonte da verdade para o resultado da recuperação
- a `Assinatura` torna-se a fonte da verdade para qualquer atualização de estado operacional resultante após a recuperação
...
- status do caso de cobrança para `recuperado`

Mas não deve transformar a história em:
- `RenewalCycle.status = bem sucedido` retroativamente para a tentativa original com falha
...
- `Assinaturas`, onde a raiz é `Assinatura`
- `Renewals`, onde a raiz é `RenewalCycle`

Então, para `Dunning`:
- a raiz deve ser `DunningCase`
- `DunningAttempt` deve ser o histórico filho
...
- um fluxo de trabalho dedicado de `start-dunning` cria ou atualiza o agregado de cobrança
...
Isso mantém:
- `Renovações` responsável pela execução da renovação
- `Dunning` responsável pela execução da recuperação
- `Assinaturas` responsável pelo estado do ciclo de vida da assinatura
...
- `DunningCase` está ancorado principalmente em um `renewal_cycle_id` de origem
...
- tentativas posteriores de recuperação com falha atualizam o mesmo `DunningCase` e criam o histórico de `DunningAttempt`, não novos casos
- `Assinatura` possui estado de relacionamento recorrente
