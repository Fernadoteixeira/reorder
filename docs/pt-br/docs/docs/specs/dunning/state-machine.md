...
- definir regras para `mark-recovered` e `mark-unrecovered` manuais
...
- `next_retry_at` por si só não descreve completamente a intenção do negócio
- isso mantém o modelo mais claro para ações manuais e raciocínio operacional

### Função de `next_retry_at`
...
- `status = open` e `next_retry_at = null` significa ativo, mas ainda não colocado em nova tentativa cronometrada
...
- `abrir -> retry_scheduled`
- `abrir -> tentando novamente`
- `abrir -> aguardando_resolution_manual`
- `aberto -> recuperado`
- `aberto -> não recuperado`
- `retry_scheduled -> tentando novamente`
- `retry_scheduled -> aguardando_manual_resolution`
- `retry_scheduled -> recuperado`
- `retry_scheduled -> não recuperado`
- `tentando novamente -> retry_scheduled`
- `tentando novamente -> awaiting_manual_resolution`
- `tentando novamente -> recuperado`
- `tentando novamente -> não recuperado`
- `awaiting_manual_resolution -> retry_scheduled`
- `awaiting_manual_resolution -> recuperado`
- `awaiting_manual_resolution -> não recuperado`
...
- a política de nova tentativa define um novo `next_retry_at`
- `attempt_count` permanece abaixo do limite final de fechamento
...
- `retry_scheduled -> abrir`
- `tentando novamente -> abrir`
- `recuperado -> aberto`
- `recuperado -> retry_scheduled`
- `recuperado -> tentando novamente`
- `recuperado -> aguardando_manual_resolution`
- `recuperado -> não recuperado`
- `não recuperado -> aberto`
- `não recuperado -> retry_scheduled`
- `não recuperado -> tentando novamente`
- `não recuperado -> aguardando_manual_resolution`
...
- a tentativa torna-se `fracassada`
...
- o atual `DunningAttempt` torna-se `bem sucedido`
- o caso transita para `recuperado`
...
- a tentativa atual torna-se `failed`
...
- a tentativa atual torna-se `failed`
- o caso transita para `não recuperado`
- `closed_at` está definido
- `next_retry_at` está limpo
...
- `status = aguardando_resolução_manual`
...
- o caso deve passar para `não recuperado`
...
- `repetir` somente se nenhuma nova tentativa estiver realmente em andamento, o que na prática deve ser tratado como bloqueado
...
Efeitos:
- transições de caso para `recuperado`
- `recovered_at` está definido
- `closed_at` está definido
- `next_retry_at` está limpo
...
Efeitos:
- transições de caso para `não recuperado`
- `closed_at` está definido
- `next_retry_at` está limpo
- `recovery_reason=marked_unrecovered_by_admin`

### Por que as ações manuais são bloqueadas durante a `nova tentativa`
...
- tentativa falha
- a tentativa torna-se `falhada`
- `attempt_count` atingiu `max_attempts`
