...
Objetivo:
- projetar o modelo final de domínio `Assinatura`
...
- relações entre módulos são tratadas através de `defineLink`
...
- Os campos necessários para filtragem e classificação administrativa devem ser armazenados explicitamente como campos de modelo, não apenas dentro de `metadados` ou blobs JSON.

## 2. Status

Nesta fase, o domínio `Subscrição` suporta:
...
Por quê:
- eles estão fora do escopo atual de `Assinaturas`
...
- classificação
- a mutação `programação-plano-mudança`
- futuras renovações
...
### `pausado_em`, `cancelado_em`, `cancelar_efetivo_em`

Necessário para:
- auditabilidade
- lidar com `pausa`
- manipulação de `cancelar`
...
- filtragem por `cliente` vinculado
- filtragem por `produto` vinculado
- filtragem por `variante` vinculada
...
1. `2.1.4`
   - implementação do módulo `subscrição`
2. `2.1.5`
   - links de módulos
3. `2.1.6`
   - migrações e índices
