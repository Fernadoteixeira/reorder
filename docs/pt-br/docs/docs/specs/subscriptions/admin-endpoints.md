...
Objetivo:
- projetar os endpoints de back-end para a visualização administrativa de `Assinaturas`
...
- Todos os endpoints estão sob o prefixo `/admin`, então eles são automaticamente somente administradores no Medusa.
- Os manipuladores de rota usam `AuthenticatedMedusaRequest`.
- Os endpoints de leitura usam `query.graph()` ou `query.index()` se a filtragem exigir a passagem de módulos vinculados.
...
- Consulta:
  ...
  - mude para `query.index()` se a filtragem por `cliente`, `produto` ou `variante` exigir módulos vinculados
...
|---|---|---|
| `GET /admin/subscrições` | leia | modelo de consulta/leitura |
| `GET /admin/subscrições/:id` | leia | modelo de consulta/leitura |
| `POST /admin/subscrições/:id/pause` | mutação | fluxo de trabalho |
| `POST /admin/subscriptions/:id/resume` | mutação | fluxo de trabalho |
| `POST /admin/subscrições/:id/cancelar` | mutação | fluxo de trabalho |
