...
A área `Planos e Ofertas` tem três conceitos distintos:
...
- `source_scope = produto`
- `source_offer_id = ID do registro do produto`
...
1. leia o registro de origem no nível da variante para `variant_id`
...
- `source_scope = variante`
- `source_offer_id = ID do registro da variante`
...
- se não existir nenhuma fonte válida, isso deve ser resolvido como `false`
...
O detalhe da fonte é o detalhe do administrador de um registro `PlanOffer` concreto.
...
A lista de administradores para `Planos e ofertas` deve permanecer baseada no registro de origem.
...
Ele deve ser validado apenas com relação à configuração efetiva resolvida.
...
- `resolution_reason = "direct_variant" | "produto_fallback" | "sem_correspondência"`
...
Isso mantém `Assinaturas` e `Planos e Ofertas` corretamente separados:
- `Planos e Ofertas` define a política atual
- `Assinaturas` armazena o estado operacional real aceito
