...
Este documento define o Admin UX para a área `Planos e Ofertas`.
...
A principal referência de UX para este recurso é a implementação de administração de `Assinaturas` existente neste plugin.
...
- `reordenar/src/admin/routes/subscriptions/page.tsx`
- `reordenar/src/admin/routes/subscriptions/[id]/page.tsx`
- `reorder/src/admin/routes/subscriptions/data-loading.ts`

## 3. Arquitetura da informação

`Planos e Ofertas` não devem ser introduzidos como uma área administrativa de nível superior.

Deve ser uma rota Admin aninhada em `Assinaturas`, semelhante a como o Medusa mostra páginas secundárias em áreas pai existentes, como:
- `Produtos`
- `Coleções`
- `Categorias`
...
Rota recomendada:
- `/assinaturas/planos-ofertas`

Configuração de rota de administrador recomendada:
- rótulo: `Planos e ofertas`
- aninhado: `/subscrições`
...
Quando o usuário abre a área `Assinaturas` na barra lateral do Admin:
- `Assinaturas` continua sendo a entrada pai
- `Planos e Ofertas` aparece como uma rota secundária aninhada abaixo dele
...
A página `Assinaturas` existente permanece responsável por:
...
A nova página `Planos & Ofertas` é responsável por:
...
Por quê:
- Os padrões Medusa Admin recomendam `Drawer` para editar/atualizar formulários
...
A página da lista deve usar a mesma estrutura geral de página de `Assinaturas`:
- `Contêiner`
...
A página deve seguir o mesmo padrão de `Assinaturas` e as orientações de habilidade do Medusa Admin:
...
CTA principal na página `Planos e ofertas`:
- `Criar`
...
- se `scope = variante`, a seleção da variante é necessária
...
Opcional:
- `Ver detalhes` somente se a página posteriormente obtiver uma rota de detalhes dedicada ou painel lateral
...
- carregamento do `DataTable`
- não deve aparecer um espaço reservado para o estado vazio durante o primeiro carregamento
...
- `Alert`
- texto conciso e adequado ao contexto
...
Para manter a consistência com a experiência do usuário (UX) existente da seção `Assinaturas`:
...
- uma rota Admin aninhada em `Subscriptions`
