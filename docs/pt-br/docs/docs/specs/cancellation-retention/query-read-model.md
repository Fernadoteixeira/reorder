...
Objetivo:
- definir o modelo de leitura do administrador para `Cancelamento e retenção`
...
- `reordenar/src/modules/cancellation/utils/admin-query.ts`
...
- `CancellationCase` não está ancorado em um ciclo de renovação concreto
...
Os resumos de cobrança e renovação devem usar enriquecimento em tempo de consulta por `subscription_id`.
...
em seguida, trate isso como filtragem de dados vinculados e introduza `query.index()` ou uma estratégia de consulta vinculada dedicada explicitamente.
...
- `CancellationCase` continua sendo a raiz do modelo de leitura
