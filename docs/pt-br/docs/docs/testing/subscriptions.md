...
A configuração de teste para `Subscrições` foi projetada para proteger o plugin nas camadas oficialmente suportadas pelas ferramentas de teste da Medusa.
...
A área `Assinaturas` é testada em duas camadas principais:
...
- `POST /store/carts/:id/subscribe` agora existe como rota dedicada de compra de assinatura
...
Para a nova funcionalidade `Assinaturas`:
...
A área `Assinaturas` é atualmente testada por meio de camadas de integração suportadas pela Medusa, em vez de automação do navegador.
