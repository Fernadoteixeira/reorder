...
Este documento descreve a estratégia de teste atual para a área `Log de atividades` no plugin `Reorder`.
...
A configuração de teste para `Registro de atividades` foi projetada para proteger a camada de auditoria de negócios nos limites oficialmente suportados pelas ferramentas de teste da Medusa.
...
A área `Registro de atividades` é testada em duas camadas principais:
...
Os auxiliares de dados de teste reutilizados pela cobertura do `Log de atividades` são definidos em:
...
Os testes `Log de atividades` preferem intencionalmente:
...
- Estabilidade e diferenciação `dedupe_key`
...
- Emissão de eventos `Subscriptions` para:
  - pausa
  - currículo
  - cancelar
...
- Emissão de eventos de `renovações` e registro de resultados
- Emissão de eventos `Cancelamento e Retenção` e presença de carga útil
...
- a implementação atual do `Registro de atividades` é validada por meio de integração de back-end e fluxos de API de administração
...
O `Log de atividades` está atualmente protegido nas camadas que são importantes para este plugin:
