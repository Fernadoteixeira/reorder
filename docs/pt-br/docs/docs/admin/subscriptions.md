...
Este documento descreve a UI Admin implementada para a área `Subscrições` no plugin `Reorder`.

Ele se concentra no comportamento da tela, nos fluxos do usuário, nas ações e no tratamento do estado da IU.

## Propósito

A interface de administração de `Assinaturas` oferece aos operadores um espaço de trabalho dedicado para:
...
- `/app/subscrições/renovações`
- `/app/subscriptions/dunning`
- `/app/subscrições/cancelamentos`

Personalização integrada da página Admin relacionada:
- `Detalhes do pedido` mostra um widget de `Assinatura` deste plugin
...
Limitação atual:
- em rotas de detalhes dinâmicos neste grupo de navegação personalizado, o estado ativo da barra lateral volta para `Assinaturas`
...
A lista também expõe `Limpar tudo` quando qualquer filtro está ativo.
...
- `Produto`
- `Próxima renovação`
- `Atualizado` como uma chave de classificação técnica de backend, mesmo que não seja mostrada como uma coluna visível
...
- `ativo` -> `Pausar`, `Cancelar`
- `pausado` -> `Retomar`, `Cancelar`
- `cancelado` -> nenhuma outra ação de mutação do ciclo de vida
...
A seção `Produto` usa um cartão vinculado no estilo Medusa que abre a página de detalhes da variante padrão do produto.
`SKU` permanece visível como um campo auxiliar separado abaixo do cartão.

Regra atual de resolução de dados:
- Os dados de exibição de `Cliente` e `Produto` são resolvidos ao vivo a partir de registros vinculados da Medusa, quando disponíveis
...
O plugin também estende a página padrão `Detalhes do pedido` da Medusa.

O widget `Assinatura` mostra:
- `Pedido de assinatura` com um cartão de assinatura vinculado quando o pedido está vinculado a uma assinatura
- rótulo de desconto de assinatura atual derivado de `pricing_snapshot`
- próxima data de renovação projetada
- `Pedido único` quando não existe link `subscription_order`
...
- `Pausa`
- `Retomar`
- `Programar mudança de plano`
- `Editar endereço de entrega`
- `Cancelar`

A disponibilidade da ação segue as mesmas regras estaduais da lista, quando relevante.

### Por que `Programar mudança de plano` vive apenas em detalhes

`Programar mudança de plano` é intencionalmente exposto na página de detalhes, não no menu da linha da lista.
...
- o evento de log de atividades resultante armazena diferenças de endereços legíveis, como `Endereço: antigo -> novo`

## 5. Seção de registro de atividades

A página de detalhes inclui uma seção dedicada `Registro de atividades` para uma assinatura.
...
- suporta `Adicionar filtro` para filtros de domínio, ator e data opcionais
- mostra as entradas `Criado de` e `Criado para` somente depois que esses filtros são adicionados no menu
...
- renderiza `Event` sem uma legenda de domínio secundário na célula da tabela
...
- erros no nível da página são renderizados inline por meio de `Alert`
...
### Por que cliente e produto ainda não têm seletores `Adicionar filtro` separados
...
A área administrativa de `Assinaturas` ainda possui ações diretas de ciclo de vida, como:
- pausa
- currículo
- cancelar

Ao mesmo tempo, o gerenciamento de rotatividade agora tem seu próprio espaço de trabalho dedicado em `Assinaturas`:
