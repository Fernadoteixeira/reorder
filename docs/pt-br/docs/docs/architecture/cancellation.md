...
A área “Cancelamento e Retenção” oferece um fluxo de gerenciamento de cancelamentos, administrado pela operadora, para assinaturas recorrentes.

A implementação atual oferece suporte a:
- acompanhamento de `cancellation_case` e `retention_offer_event`
- início e continuidade de um caso de cancelamento para assinaturas elegíveis
- aplicação de ofertas de retenção por meio de pausa, desconto e bônus
- finalização do cancelamento com os dados obrigatórios sobre o motivo da baixa
- visualizações de lista e detalhes do administrador aninhadas em `Assinaturas`
- integração com `Assinaturas`, `Renovações` e `Cobranças em atraso`
...
- os fluxos de trabalho são responsáveis pela criação de casos, aplicação de ofertas, finalização de cancelamentos e atualizações de motivos
...
- a tarefa do agendador calcula métricas operacionais e gera logs estruturados para picos de rotatividade que exigem alertas
...
- cada `CancellationCase` está vinculado a um único `subscription_id`
- uma assinatura pode ter vários casos históricos, mas apenas um caso ativo por vez no MVP
- as ações concretas de retenção são armazenadas separadamente como `RetentionOfferEvent`, que só permite adições
- `Subscription` continua sendo a fonte do estado do ciclo de vida, enquanto `CancellationCase` continua sendo a fonte do estado dos processos de cancelamento e retenção
...
As migrações atuais e a configuração do modelo otimizam `Cancelamento e retenção` para:
...
- oferta de filtragem por `decision_status`

## 3. Semântica de Integração e Propriedade

`Cancelamento e Retenção` está integrado com `Assinaturas`, `Renovações` e `Cobrança` no tempo de execução atual.
...
- aplicar uma `discount_offer` ou `bonus_offer` mantém a assinatura ativa e fecha o caso como `retido`
- cancelamento final move a assinatura para `cancelled`, define `cancel_efficient_at` e limpa `next_renewal_at`
- `DunningCase` ativo pode coexistir com `CancellationCase` ativo
- As `renovações` permanecem proprietárias do `RenewalCycle`, enquanto o cancelamento afeta a elegibilidade para renovação através do estado do ciclo de vida da assinatura

Isso significa:
- `CancellationCase` possui o estado do processo de rotatividade
- `RetentionOfferEvent` possui histórico de ofertas
- `Subscrição` possui materialização do ciclo de vida
- Histórico de execução do próprio ciclo de `renovações`
...
- `subscription_id` suporta indexação, filtragem e verificações invariáveis
...
- ajudantes de normalização e mapeamento DTO em `src/api/admin/cancellations/utils.ts`
...
- filtragem opcional `offer_type` através do histórico de ofertas

### Fluxo detalhado

Para a visualização detalhada:
1. a UI do administrador solicita `GET /admin/cancellations/:id`
2. A rota resolve o caso por meio do auxiliar de consulta detalhada
3. O resumo da assinatura vinculada foi resolvido
4. Os resumos opcionais de cobrança e renovação são resolvidos em `subscription_id`
...
No tempo de execução atual, `Cancellation & Retention` Admin lê usando `CancellationCase` como raiz da consulta de origem.
...
- `subscrição` é o principal enriquecimento vinculado
- Os resumos de `cobrança` e `renovação` são apenas no contexto do tempo de consulta
...
- `caso de início de cancelamento`
- `aplicar-oferta-de-retenção`
- `finalizar-cancelamento`
- `motivo do cancelamento da atualização`
...
- fechar o caso como `pausado` ou `retido`
...
- computando `cancelar_efetivo_at`
- atualizando a assinatura para `cancelada`
- limpando `next_renewal_at`
- chamando `garantir o próximo ciclo de renovação`
- fechando o caso como `cancelado`

### Fluxo de trabalho do motivo

`update-cancellation-reason` são os metadados manuais e a mutação de classificação.

É responsável por:
- atualizando o `motivo`
...
A API Admin expõe rotas personalizadas dedicadas às páginas `Cancelamento e retenção`.
...
- campos explícitos como `finalized_by`, `decided_by` e campos de ator específicos do fluxo de trabalho
- anexar entradas de metadados `manual_actions` apenas com `quem / quando / por que / dados`
...
- `src/modules/cancellation/utils/operacional-metrics.ts`
- `src/jobs/process-cancellation-operacional-metrics.ts`
...
`Cancelamento e Retenção` atualmente possui:
