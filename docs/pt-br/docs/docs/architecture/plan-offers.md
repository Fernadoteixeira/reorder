...
Este documento descreve a arquitetura atual da área `Planos & Ofertas` no plugin `Reorder`.

Ele se concentra no sistema implementado e não nas suposições iniciais do projeto.

## Meta

A área `Planos e Ofertas` fornece a camada de configuração comercial para produtos habilitados para assinatura no Admin.
...
- resolver a configuração de assinatura efetiva com prioridade `variante > produto`
...
- o módulo de domínio possui o modelo de dados `plan_offer` e a persistência
...
`frequency_intervals` armazena o conjunto nivelado de nomes de intervalos usados ​​para suportar uma filtragem mais rápida no Admin.
...
A área `Planos & Ofertas` distingue entre:
...
Isso significa que a configuração efetiva usa a semântica `variant > product`.
...
- resolução de configuração efetiva em `src/modules/plan-offer/utils/efficient-config.ts`
...
4. A camada de consulta lê registros `plan_offer` através de `query.graph(...)`
...
1. a UI do administrador solicita `GET /admin/subscription-offers/:id`
...
Os fluxos de trabalho são o limite de mutação da área `Planos e Ofertas`.

A camada de mutação atual é construída em torno de três fluxos de trabalho:
- `criar ou atualizar-plano-oferta`
- `oferta-plano de atualização`
- `alternar-plano-oferta`
...
Lacuna atual:
- As mutações `Planos e Ofertas` ainda não emitem eventos de auditoria e não são visíveis no `Registro de Atividades`.
...
A API Admin expõe rotas personalizadas dedicadas à página `Planos e ofertas`.
...
A UI Admin é implementada como uma página de administração Medusa personalizada para `Planos e ofertas`.

A IU atual inclui:
- uma página de lista apoiada por Medusa `DataTable`
- criar fluxo com um `FocusModal`
- edite o fluxo com uma `Drawer`
...
`Planos & Ofertas` é a camada de configuração comercial utilizada pela área de `Assinaturas`.

O ponto de integração implementado é o fluxo de mudança de plano de assinatura.

Durante `mudança de plano de cronograma`:
...
Isso significa que `Planos e Ofertas` já influencia o que pode ser agendado para uma assinatura no Admin.

O limite de propriedade permanece claro:
- `Planos e Ofertas` possui política de ofertas
- `Assinaturas` possui o estado do ciclo de vida da assinatura e alterações pendentes no plano
...
- integração de nível de fumaça com `Assinaturas`
...
`Planos e Ofertas` atualmente possui:
