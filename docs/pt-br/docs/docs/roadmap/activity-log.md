...
Este documento descreve o roteiro de acompanhamento para a área `Registro de atividades` após a implementação atual da v1.
...
`Log de atividades` v1 é implementado e inclui:
- armazenamento `subscription_log` somente anexado
...
- transformando `Registro de atividades` em telemetria geral
