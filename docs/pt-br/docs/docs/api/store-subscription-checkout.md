...
Objetivo:
- resolver a configuração efetiva de `Planos e Ofertas` para o item de linha de assinatura
...
- usa `descrição = "Desconto de assinatura"`
...
- `line_item.metadata.frequency_interval: "semana" | "mês" | "ano"`
- `line_item.metadata.frequency_value: número inteiro positivo`

Metadados opcionais do carrinho:

- `cart.metadata.purchase_mode: "subscrição"`
