...
Quando a aprovação não é necessária, a API retorna `status = null` dentro do resumo da aprovação.
...
- `status_aprovação?: string | string[]`
- `agendado_de?: string`
...
- `efficient_scheduled_for` é a data de entrega projetada mostrada no Admin quando a assinatura vinculada atualmente tem um próximo ciclo ignorado
...
- `400 dados_inválidos`
  A política atual de "Planos e Ofertas" bloqueia a aplicação da alteração pendente.
...
Ao contrário da aprovação, o `motivo` é exigido no contrato de API atual.
...
Esses campos são disponibilizados por meio de `metadata` na resposta de detalhes.

Os consumidores do Admin desta API estão implementados em:
