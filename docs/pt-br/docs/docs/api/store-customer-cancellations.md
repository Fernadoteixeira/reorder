...
- a carga inclui `scheduled_plan_change` quando existe uma variante pendente ou atualização de cadência
- a carga útil inclui `next_renewal_at` e `efficient_next_renewal_at` projetado
...
- a cadência é validada em relação aos `Planos e Ofertas` ativos
...
- a variante alvo deve pertencer ao produto de assinatura e ser permitida por `Planos e Ofertas` ativos
...
Resposta:
- carga útil mínima `cancellation_case` com `id`, `status`, `subscription_id` e campos de motivo enviados
