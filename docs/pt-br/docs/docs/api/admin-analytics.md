...
Exemplos:
- semanalmente: `intervalo = "semana", valor = 1`
- a cada 2 semanas: `intervalo = "semana", valor = 2`
- mensal: `intervalo = "mês", valor = 1`
...
- `id_do_produto?: string | string[]`
- `frequência?: string | string[]`
- `group_by?: "dia" | "semana" | "mês"`
- `fuso horário?: "UTC"`

Notas:
- `date_from` e `date_to` são carimbos de data/hora semelhantes a ISO ou strings de data interpretadas pelo validador da API.
...
- A análise do MVP rejeita valores de fuso horário não `UTC`.
...
- Os tokens de `frequência` devem corresponder a `semana:n`, `mês:n` ou `ano:n`
- valores de `fuso horário` não suportados são rejeitados
...
- `kpis` ainda retorna todas as chaves KPI
...
- `export` retorna uma carga válida com `linhas` vazias
...
- a resposta sempre inclui todas as chaves de KPI suportadas para MVP, mesmo quando alguns valores são `nulos`
- `MRR` e `LTV` podem ser resolvidos como `nulos` quando o conjunto de dados selecionado não tiver um único contexto de moeda válido ou quando o instantâneo da receita estiver incompleto para o cálculo do MVP
...
- `created_subscriptions_count` é sempre retornado como buckets UTC diários, mesmo se `group_by` for `week` ou `month`
...
Para MVP, o contrato de exportação é síncrono e suporta `csv` e `json`.
...
- `colunas` são determinísticas e definem a ordem de exportação nivelada
- `rows` representam o conjunto de dados nivelado pronto para exportação
...
- As células de exportação `MRR` e `LTV` podem ser `nulas` quando o intervalo subjacente não tem uma base de receita válida em moeda única
...
- falha parcial não altera automaticamente o status HTTP para `500`
