...
A interface de usuário administrativa do `Analytics` oferece aos operadores um painel voltado para relatórios sobre KPIs e tendências do comércio recorrente.
...
A interface do usuário é implementada como uma página personalizada do Medusa Admin aninhada em `Subscriptions`.
...
- a página está agrupada na área de administração `Subscriptions`
...
- `group_by` assume por padrão o valor de `day`
- a semântica do fuso horário é fixada em `UTC` na versão MVP
...
- As métricas de porcentagem utilizam a precisão do KPI configurada na carga útil da resposta
...
- a seleção da métrica determina qual série é destacada
...
- os intervalos `day`, `week` e `month` usam todos o `UTC`
...
A página “Analytics” apresenta uma ação `Export` com:
- `CSV`
- `JSON`
...
Atualmente, existe integração de invalidação para:
