...
Objetivo:
- definir o modelo de leitura Admin para `Plans & Offers`
...
A camada de leitura `Plans & Offers` deve distinguir entre:
...
- recuperando um registro de origem `PlanOffer` por `id`
...
- aplicando substituto `variant > product`
- retornando um derivado `ProductSubscriptionConfig`
...
- `PlanOffer` permanece a raiz do registro de origem para visualizações Admin CRUD
...
- `product_id`
- `variant_id`
- `frequency` a `frequency_intervals`
- `q` através de campos de origem como `name`

Interpretação recomendada de `frequency`:
- `frequency=month` corresponde a registros cujo `frequency_intervals` contém `month`
...
- resolver a origem habilitada no nível do produto até `product_id`
...
- caso contrário, resolva a origem habilitada no nível do produto até `product_id`
...
Isso reflete a estrutura já usada em `Subscriptions`, onde o mapeamento é explícito e não oculto dentro do código de rota.
...
Esta estrutura mantém a implementação próxima do estilo já utilizado por `Subscriptions`.

## 20. Recomendação final

O modelo final de leitura do administrador para `Plans & Offers` deve ser:

1. Lista auxiliar para registros de origem `PlanOffer`.
2. Auxiliar detalhado para um registro de origem `PlanOffer` mais resumo efetivo.
