...
Objetivo:
- definir os links dos módulos exigidos por `Plans & Offers`
...
`Plans & Offers` precisa de dois links de módulo:
...
- um registro `PlanOffer` pertence ou é direcionado a um produto
...
No nível do domínio:
- `scope = product` significa que não há destino variante
- `scope = variant` significa que deve haver um alvo variante
...
- segue o padrão prático Medusa já usado na área `Subscriptions` do plugin
...
- os filtros de lista devem ter como alvo principal os campos diretos em `PlanOffer`
...
- recupere o registro de origem `PlanOffer` primeiro
...
Quando um `PlanOffer` tem como alvo um escopo em nível de produto:
- O administrador deve exibir o título do produto
- a exibição da variante deve ser renderizada como `All variants` ou rótulo de nível de origem equivalente
...
- não presuma que `query.graph()` possa filtrar módulos vinculados da raiz de origem em todos os casos
...
- resolve a partir dos registros de origem `PlanOffer`
...
- ao criar um `PlanOffer` de nível de variante, crie o link do produto e o link da variante
- ao atualizar um alvo em um fluxo permitido futuro, os links devem ser atualizados adequadamente
- ao excluir ou substituir links, a direção do link deve corresponder à ordem `defineLink(...)` original
...
A estratégia de link final para `Plans & Offers` deve ser:
...
6. Mantenha a lista/detalhe do administrador enraizada em `PlanOffer`.
