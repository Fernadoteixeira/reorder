# Reorder: Renovações Admin UI and API Spec
...
Pendente changes are represented as a normalized preview of the subscription change that may be applied during renewal:
...
Renovação detail extends the list record with:
