...
Objetivo:
- definir os links dos módulos exigidos por `Renewals`
- definir a direção do link e a semântica do relacionamento
- definir como os dados de assinatura e pedido vinculados devem ser lidos no Admin
...
- o registro de origem pertence ao módulo `renewal` personalizado
...
Também mantém o módulo de renovação alinhado com a direção do link estabelecido já utilizado por `Subscriptions`.
...
- o resumo final do ciclo deve apontar para a ordem vencedora gerada através de `generated_order_id`

Isso não impede tentativas de armazenar também `order_id` como um campo de diagnóstico escalar.
...
- corresponde ao padrão prático já utilizado na área `Subscriptions` do plugin
...
- os filtros de lista devem ter como alvo principal os campos diretos em `RenewalCycle`
...
- recupere o registro de origem `RenewalCycle` primeiro
