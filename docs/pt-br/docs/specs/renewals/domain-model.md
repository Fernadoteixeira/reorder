...
A área `Renewals` possui dois níveis conceituais:
...
- armazenar os campos de conveniência atuais utilizados pelo processamento da fila e pelo Admin

A `RenewalCycle` não se responsabiliza por:
...
## 5. Campos `RenewalCycle` regulares
...
`applied_pending_update_data` descreve o instantâneo da alteração pendente que foi efetivamente utilizado durante o processamento da renovação.
...
- histórico de erros detalhado pertence a `RenewalAttempt`
...
- `pending` significa que a aprovação é necessária e ainda não foi decidida
- `approved` significa que o ciclo pode aplicar alterações pendentes
- `rejected` significa que o ciclo não deve aplicar alterações pendentes
