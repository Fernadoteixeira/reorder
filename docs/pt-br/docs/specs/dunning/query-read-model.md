...
O modelo de leitura deve enriquecer os registros com um resumo do pedido vinculado quando `renewal_order_id` existir.
...
O modelo inicial de leitura do administrador `Dunning` não deve exigir `query.index()` por padrão.
...
- `attempt_count`
- `max_attempts`
- `next_retry_at` períodos
- `last_attempt_at` períodos
- `recovered_at` períodos
- `closed_at` períodos
...
- `Subscription` é apenas contexto operacional vinculado
- `RenewalCycle` é apenas contexto de evento de origem vinculado
- `Order` está vinculado apenas ao contexto de pagamento/pedido
