...
A programação de nova tentativa `Dunning` deve usar:
...
Onde:
- `intervals` são compensações de repetição em minutos
...
- permite que o agendador use um campo devido explícito: `next_retry_at`
...
Depois de um `DunningAttempt` com falha:
- a próxima tentativa deve ser agendada em relação ao `finished_at` da tentativa fracassada
...
- o caso `retry_schedule.intervals`
...
- após tentativa falhada número `n`:
  - se `intervals[n]` existir, `next_retry_at = failed_attempt.finished_at + intervals[n]`
...
Significado:
- o caso poderá executar até três tentativas reais de recuperação de pagamento `DunningAttempt`
- assim que a terceira tentativa falhar, o caso deverá ser encerrado como `unrecovered`
...
## 8. Relacionamento entre `intervals` e `max_attempts`
...
- o limite efetivo de novas tentativas até `max_attempts`
...
- recalcular `next_retry_at` imediatamente a partir da nova política
...
- o plugin já segue padrões explícitos orientados a fila em `Renewals`
...
- `attempt_count = 1`
- o próximo intervalo é `3 days`
- `next_retry_at = failed_attempt.finished_at + 3 days`
- caso permanece `retry_scheduled`
...
- o administrador substitui a programação para `[2880, 10080]`
...
- armazene apenas uma data de vencimento concreta em `next_retry_at`
- política padrão:
