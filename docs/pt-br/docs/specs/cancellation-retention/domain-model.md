...
Este documento abrange as etapas `2.5.3`, `2.5.4`, `2.5.5` e `2.5.6` de `documentation/implementation_plan.md`.

Objetivo:
- definir o contrato de domínio para `CancellationCase`
- definir o contrato de domínio para `RetentionOfferEvent`
- definir a taxonomia e semântica de `reason_category`
...
- JSON é apropriado para `metadata`, não para máquina de estado primária ou campos de relatório
...
A área `Cancellation & Retention` possui dois níveis conceituais:
...
- uma entidade de histórico filho depois: `RetentionOfferEvent`
...
- armazene todo o histórico de ofertas de retenção apenas em `CancellationCase.metadata`

Por que é pior:
- mais difícil de inspecionar operacionalmente
- auditabilidade mais fraca
- renderização de linha do tempo mais difícil
- menos alinhado com o padrão `Renewals` e `Dunning` estabelecido
...
## 5. Campos `CancellationCase` regulares
...
- preserva o mesmo padrão prático da Medusa já usado em `Subscriptions`, `Renewals` e `Dunning`
...
`reason` é o motivo comercial no nível do caso inserido ou selecionado para esta jornada de cancelamento.
...
Nota importante:
- `reason` não é a categoria de relatório normalizada
...
## 10. `reason_category` decisão de taxonomia

Para MVP, `reason_category` deve ser uma predefinição fixa ou enumeração em `CancellationCase`, não um dicionário gerenciado pelo administrador.

### Decisão final

Abordagem recomendada:
- mantenha `reason_category` como uma predefinição estável no agregado
- mantenha `reason` como campo de suporte de texto livre
...
- `Cancellation & Retention` ainda está definindo seu contrato de processo principal
...
- eles podem posteriormente suportar heurísticas de recomendação em `smart-cancellation`
...
## 12. Relacionamento entre `reason` e `reason_category`
...
`reason` não deve ser tratado como a principal fonte de relatórios estruturados.

## 13. Regra de relatório para texto livre `reason`
...
- os operadores podem fornecer opcional ou condicionalmente `reason`
- relatórios agregados em `reason_category`
- o texto livre `reason` permanece apoiando o contexto qualitativo
...
- usar `reason_category = other`
- preserve a nuance em `reason`
...
Nota importante:
- o histórico real da oferta e as ações aplicadas devem estar disponíveis posteriormente em `RetentionOfferEvent`
...
É uma ação de retenção, não apenas uma alternância direta do ciclo de vida de `Subscriptions`.
...
Como recomendação:
- `recommended_action = pause_offer` significa que o processo atualmente considera pausar a melhor opção de próximo salvamento

### Nível de oferta concreto

Como um evento concreto:
- a oferta deverá ser representada por um `RetentionOfferEvent`
...
- não proponha `pause_offer` para uma assinatura `cancelled`
- não trate uma assinatura já `paused` como um novo alvo normal de oferta de retenção de pausa
...
Como recomendação:
- `recommended_action = discount_offer` significa que o processo atualmente considera uma concessão de preços a melhor próxima opção de salvamento
...
- o desconto não deve substituir ou sofrer mutação global `Plans & Offers`
...
- uma recomendação sobre `CancellationCase` nunca é suficiente por si só para ser considerada uma oferta concreta
- cada ação de salvamento concreta deve ser armazenada como um `RetentionOfferEvent` separado
...
- o impacto do ciclo de vida deve ser materializado posteriormente por meio de alterações apoiadas pelo fluxo de trabalho em `Subscription`

## 21. Limite com outros domínios

As ações de retenção devem permanecer com escopo definido para `Cancellation & Retention`.

Eles não devem substituir a propriedade de:
- `Subscriptions` para o estado final do ciclo de vida
- `Plans & Offers` para política de oferta global
- `Renewals` para execução do ciclo
...
`cancellation_effective_at` é o tempo efetivo no nível do processo escolhido para o resultado final do cancelamento.
...
- não substitui `Subscription.cancel_effective_at`
...
## 27. Campos `RetentionOfferEvent` regulares
...
## 28. Por que `cancellation_case_id` deveria ser um campo escalar
...
Por quê:
- o formato da carga útil varia em `offer_type`
...
Nota importante:
- `decision_reason` não é o mesmo que a rotatividade `reason` em `CancellationCase`
- `CancellationCase.reason` explica por que a assinatura entrou em tratamento de cancelamento
- `decision_reason` explica por que um evento de oferta específico terminou daquela maneira

## 33. `decided_at`, `decided_by` e `applied_at`
...
Isso é intencionalmente separado de `decided_at`.
...
Os dados a seguir não devem ser armazenados como campos mutáveis ​​principais em `CancellationCase`:
...
Por quê:
- pertencem a outros agregados ou ao futuro `RetentionOfferEvent`
- duplicá-los enfraqueceria os limites da fonte da verdade já definidos em `2.5.2`
...
### `metadata` não deve ser usado para:

- filtros de administração primários
- transições de estado atual
- campos críticos para relatórios

Isso mantém o modelo de leitura futura alinhado com os mesmos princípios já usados ​​em `Subscriptions`, `Renewals` e `Dunning`.
