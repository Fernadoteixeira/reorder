# Reorder: Cancelamento e Retenção Module Impact Semantics Spec
...
Agendado `RenewalCycle` records are not deleted, re-owned, or directly mutated just because a `CancellationCase` was opened.
...
- Ativo dunning and active cancellation may coexist.
