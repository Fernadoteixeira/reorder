...
Objetivo:
- definir a divisão da fonte da verdade entre `Subscriptions`, `Renewals`, `Dunning` e `Cancellation & Retention`
- decidir a qual estado pertence `Subscription`
- decidir a qual estado pertence `CancellationCase`
...
Isso inclui:
- se a assinatura é `active`, `paused`, `cancelled` ou `past_due`
- o produto ativo e a variante
- a cadência ativa
- agendar âncoras como `next_renewal_at`
...
- se esse ciclo é `scheduled`, `processing`, `succeeded` ou `failed`
...
`Cancellation & Retention` é o proprietário do processo de desligamento e salvamento quando um operador inicia o tratamento do cancelamento.
...
A fonte da verdade sobre o ciclo de vida do cliente e o estado da assinatura operacional ativa permanece `Subscription`.
...
- alterações de assinatura pendentes, mas ainda não aplicadas
- carimbos de data e hora finais do ciclo de vida, como `paused_at`, `cancelled_at` e `cancel_effective_at`
...
`Cancellation & Retention` não deve substituir:
- `RenewalCycle.status`
- estado de aprovação
...
A fonte da verdade para o estado de recuperação de pagamento permanece `DunningCase`.
...
## 3. O que pertence a `Subscription`

`Subscription` deve continuar a possuir:
...
## 7. Relacionamento com ativo `DunningCase`
...
- `Cancellation & Retention` responde se o operador está gerenciando offboarding ou salvando ações
...
## 8. Relacionamento com futuro `RenewalCycle`

`Cancellation & Retention` não deve possuir `RenewalCycle`.

`Renewals` permanece o proprietário de:
- registros de fila
- estado de execução
- histórico de tentativas
...
O tratamento de cancelamentos deve influenciar futuras renovações indiretamente por meio do estado `Subscription` e da materialização explícita do fluxo de trabalho, e não pela apropriação dos ciclos de renovação.

Isso significa:
- a abertura de um caso de cancelamento não substitui ou exclui a propriedade da renovação
- os resultados finais do processo, como `paused` ou `canceled`, devem posteriormente se materializar no estado do ciclo de vida da assinatura
...
### 9.1 Estado de propriedade de `Subscription`
...
### 9.2 Estado propriedade de `CancellationCase`
...
- detalhes do histórico de ofertas apenas anexados que devem pertencer a `RetentionOfferEvent`

### 9.3 Estado propriedade de `RetentionOfferEvent`
...
### 9.4 Estado propriedade de `RenewalCycle`
...
### 9.5 Estado propriedade de `DunningCase`
...
- `Subscription` continua sendo a fonte da verdade para o ciclo de vida e o estado de assinatura operacional ativa
- `CancellationCase` torna-se a fonte da verdade para o estado do processo de cancelamento e retenção
- `RetentionOfferEvent` torna-se o histórico apenas anexado de ofertas e decisões de retenção
...
- os registros futuros de `RenewalCycle` permanecem propriedade de `Renewals`, enquanto o cancelamento os afeta indiretamente por meio do estado final da assinatura materializada

Isso dá à futura área `Cancellation & Retention` um limite claro:
