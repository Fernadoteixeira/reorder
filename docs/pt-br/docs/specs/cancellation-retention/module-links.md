...
Objetivo:
- definir os links dos módulos exigidos por `Cancellation & Retention`
...
`Cancellation & Retention` precisa de um link de módulo obrigatório no MVP:
...
- `CancellationCase` não está ancorado em um concreto `RenewalCycle`
- `CancellationCase` não está ancorado em um concreto `DunningCase`
- `Renewals` e `Dunning` permanecem domínios operacionais separados com seus próprios agregados
- o contrato administrativo atual pode ser satisfeito pelo enriquecimento do tempo de consulta com base em `subscription_id`
...
- para a entidade `subscription` vinculada
...
Também mantém `Cancellation & Retention` alinhado com a direção do link estabelecida já usada em `Renewals` e `Dunning`.
...
- corresponde ao padrão prático já usado em `Renewals` e `Dunning`
...
- os filtros de lista devem ter como alvo principal os campos diretos em `CancellationCase`
...
- recupere o registro de origem `CancellationCase` primeiro
